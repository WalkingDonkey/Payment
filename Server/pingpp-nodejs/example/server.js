...
// 配置 API Key 和 App ID
// 从 Ping++ 管理平台应用信息里获取
var API_KEY = "sk_test_mDen5SSWzLOCDCarXP0OyPS0" // 这里填入你的 Test/Live Key(my test key applied in Ping++ console)
var APP_ID = "app_54i5aLLif18Cvbvj" // 这里填入你的应用 ID (app id I applied in Ping++ console)
...
var createPayment = function(channel, amount, client_ip, open_id, cb){
  ...
  switch (channel) {
    case 'alipay_wap':
      extra = {
        'success_url': 'http://127.0.0.1:8010/success' // No use yet
      };
      break;
    case 'upacp_wap':
      extra = {
...
http.createServer(function (req, res) {
  ...
  req.addListener("end", function () {
    ...
    switch (req.url) {
      // Currently only add pay function here, you can add other functions such as refund, retrieve and etc. here.

      case "/pay":
        // 创建 charge
        // var client_ip = req.connection.remoteAddress;
        var client_ip = "127.0.0.1"; // use localhost as test client;
        var params;
        try {
